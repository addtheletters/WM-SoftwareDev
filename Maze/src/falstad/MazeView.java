...
public class MazeView extends DefaultViewer {
	...
	void redrawFinish(Graphics gc) {
		...
		if(driver != null){
			centerString(gc, fm, "Driver made it!", 100);
		}
		else{
		...
		if(driver != null){
			centerString(gc, fm, "Driver: " + driver.getClass(), 200);
			centerString(gc, fm, "Steps taken: " + driver.getPathLength(), 230);
			centerString(gc, fm, "Energy used: " + driver.getEnergyConsumption(), 245);
		}
