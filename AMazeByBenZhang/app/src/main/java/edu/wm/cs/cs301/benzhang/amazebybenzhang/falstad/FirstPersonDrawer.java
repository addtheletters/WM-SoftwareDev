...
import edu.wm.cs.cs301.benzhang.amazebybenzhang.generation.BSPBranch;
import edu.wm.cs.cs301.benzhang.amazebybenzhang.generation.BSPLeaf;
import edu.wm.cs.cs301.benzhang.amazebybenzhang.generation.BSPNode;
...
import edu.wm.cs.cs301.benzhang.amazebybenzhang.generation.MazeBuilder;
import edu.wm.cs.cs301.benzhang.amazebybenzhang.generation.Seg;
 ...
 */
public class FirstPersonDrawer extends DefaultViewer implements NeedDimensions {
    private static final String TAG = "FirstPersonDrawer";
	...
	private int map_unit = Constants.MAP_UNIT;
	private int step_size = map_unit/Constants.INTERMEDIATE_STEPS;
	...
	final int viewz = Constants.VIEW_Z;  // constant from MazeController.java
	...
	public FirstPersonDrawer(int width, int height, int map_unit, int step_size, Cells seencells, BSPNode bsp_root) {
        ...
        //Log.v("FPDrawer", "width is " + width);
        //Log.v("FPDrawer", "height is " + height);
...
//				RenderingHints.VALUE_INTERPOLATION_BILINEAR);
	...
	private void drawAllSegmentsOfASector(BSPLeaf n) {
		//Log.v("FPDrawer", "drawing all segments of " + n);
	...
	private void drawSegment(Seg seg) {
        //Log.v("FPDrawer", "drawing segment " + seg);
		...
		int z1 = 0;
		int z2 = Constants.MAX_VIEW_Z; // wtf

		drawrect_ct++; // debug, counter

//        Log.v(TAG, "drawSegment viewx is " + viewx);
//        Log.v(TAG, "drawSegment viewy is " + viewy);
//        Log.v(TAG, "drawSegment viewz is " + viewz);
		ox1 -= viewx; 
		y1 -= viewy; 
		z1 -= viewz;
		ox2 -= viewx; 
        ...
        if (!clip3d(rp)){
            //Log.v(TAG, "Failed clip3d with " + x1 + "," + z1 + "," + x2 + "," + z2 + "\n");
            return;
        }
        else{
            //Log.v(TAG, "Passed clip3d with " + x1 + "," + z1 + "," + x2 + "," + z2 + "\n");
        }
...
//        Log.v(TAG, "backface zscale " + zscale + ", viewwidth " + view_width);
//        Log.v(TAG, "backface rp.x1 " + rp.x1 + ", rp.z1 " + rp.z1);
//        Log.v(TAG, "backface rp.x2 " + rp.x2 + ", rp.z2 " + rp.z2);
		if (x1 >= x2) /* reject backfaces */{
            //Log.v(TAG, "failed backface with " + x1 + "," + x2);
            return;
        }
        else{
            //Log.v(TAG, "passed backface with " + x1 + "," + x2);
        }
		int x1i = x1;
		int xd = x2-x1;
        ...
        //Log.v(TAG, "reached loop, rset is " + rset);
		while (x1i <= x2) {

//            Log.v(TAG, "x1i is " + x1i + ", x2 is " + x2);
			...
			if (!rset.intersect(p)) {
//                Log.v(TAG, "no intersect of rset and x, breaking");
                break;
            }
			x1i = p.x;
			int x2i = p.y;
...
//        Log.v("FirstPersonDrawer", "reported dimensions " + width + "," + height);
        this.view_width = width;
        this.view_height = height;
        this.zscale = view_height / 2;
    }
}
