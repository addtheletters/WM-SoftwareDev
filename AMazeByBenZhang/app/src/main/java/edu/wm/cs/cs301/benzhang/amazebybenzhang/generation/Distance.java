...
public class Distance {
	...
	/**
	 * Displays the distances to the exit of all maze cells in a neat grid corresponding to how mazes appear in the GUI
	 */
	public void prettyPrint(){
		int[][] allDists = getDists();
		for(int i = 0; i < allDists[0].length; i++){
			for(int j = 0; j < allDists.length; j++){
				System.out.print(allDists[j][allDists[0].length - i - 1] + "\t");
			}
			System.out.print("\n");
		}
	}
