...
public interface Robot {
	...
	public enum Turn { LEFT, RIGHT, AROUND;
		/**
		 * Describes the direction that will result from this turn occurring.
		 * @return the direction the turn will result in this robot facing
		 */
		protected Direction direction(){
			...
			default:
				System.err.println("BasicRobot: Unknown turn");
	...
	public enum Direction { LEFT, RIGHT, FORWARD, BACKWARD;
		/**
		 * Describes the direction opposite to the current value.
		...
		public Direction opposite() {
			...
			case RIGHT: 
				return Direction.LEFT;
			default:
				throw new RuntimeException("Unknown Direction enum type");
			}
		}
		
		/**
		 * Describes the turn that is needed to end up facing in this direction.
		 * @return the direction the turn will result in this robot facing
		 */
		protected Turn turn(){
			switch(this){
			...
			case RIGHT:
				return Turn.RIGHT;
			default:
				System.out.println("Robot: Direction: forwards direction requires no turn");
				return null;
			}
		}
		 ...
		 * @param robot the robot whose direction should be checked
		 * @return the cardinal direction
		 */
		protected CardinalDirection worldDirectionOf( Robot robot ){
			switch(this){
			case FORWARD:
				return robot.getCurrentDirection();
			case BACKWARD:
				return robot.getCurrentDirection().oppositeDirection();
			case LEFT:
				return robot.getCurrentDirection().rotateCounterCW();
			case RIGHT:
				return robot.getCurrentDirection().rotateClockwise(); // TODO make sure these aren't messed up...
			default:
				System.err.println("CardinalDirection#worldDirectionOf: Unknown direction");
				return robot.getCurrentDirection();
			}
		}
	};
