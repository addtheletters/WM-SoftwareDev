 ...
 *
 */
public class BasicRobot implements Robot, Parcelable{
	
	/*
	 * Constants for energy costs and battery levels
	 */
	public static final float INITIAL_BATTERY = 2500;
	...
	protected BasicRobot(Parcel in) {
		battery = in.readFloat();
		stopped = in.readByte() != 0;
		odometer = in.readInt();
	}

	public static final Creator<BasicRobot> CREATOR = new Creator<BasicRobot>() {
		@Override
		public BasicRobot createFromParcel(Parcel in) {
			return new BasicRobot(in);
		}

		@Override
		public BasicRobot[] newArray(int size) {
			return new BasicRobot[size];
		}
	};
	...
	public void move(int distance, boolean manual) {
        ...
        for (int i = 0; i < distance; i++) {
            boolean successfulStep = attemptStep(back);
            if (successfulStep) {
                odometer ++;
                continue;
            }
	 ...
	 */
	protected boolean attemptStep(boolean backward){
...
//			maze.checkFinished();
    ...
    /**
     * Get the steps this robot has traversed.
...
//	
	...
	public void setBatteryLevel(float level) {
		...
		this.battery = level;
		if(maze != null){
            maze.reportEnergyLevel( (int)( 100 * level / INITIAL_BATTERY ) );
        }
	}
	...
	protected boolean useEnergyIfAble(float energy){
		if(getBatteryLevel() >= energy){
			...
			if(getBatteryLevel() <= 0){
                Log.v("BasicRobot", "Robot stopped due to not having enough energy " + energy);
                stop();
			}
	 ...
	 */
	private boolean isBlocked(boolean backwards){
		Point here = getCurrentPoint();
...
//			}
//			else{
			checkpos.offset(checkdir.getDirection()[0], checkdir.getDirection()[1]); // TODO verify offset works the same as java's awt translate
//			}
			distance ++;
	...
	public void writeToParcel(Parcel parcel, int flags) {
		...
		parcel.writeByte((byte) (stopped ? 1 : 0));
